...
class Students extends CI_Controller {
...

    {
        ...
        if($this->input->get('coop_status_id') && is_numeric($this->input->get('coop_status_id'))) {
            $this->Student->coop_status_id = $this->input->get('coop_status_id');
        }
...

    {
        ...
        if($this->form_validation->run() != false){
            ...
            foreach($this->input->post('students') as $student_id) {
                ...
                if ($status_type == 1) {
                    $this->Job->update_company_status_when_officer_change_status($student_id);
                    $this->Job->update_student_status_when_officer_change_status($student_id); 
